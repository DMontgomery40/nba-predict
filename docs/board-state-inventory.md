...
| `game_states`                  | append-only, per capture                                           | live status (`scheduled` / `in-play` / `final` / `postponed` / `cancelled`), period, clock, home/away score, `started_at`, `final_at`. Source of clock/score/margin context, but not the preferred incident-time anchor when play-by-play `time_actual` is available.                                                                       |
...
- **fanduel** — not currently ingested.
- **draftkings** — not currently ingested.
