...
- The live whole-board tripwire now runs the bake-off winner end-to-end across alerts, replay, and `/api/v1/research/board-volatility`: 60-second whole-board quote buckets, intensity `Σ |Δ implied probability| * log1p(volume)`, trailing `median + 3*MAD` over the prior 20 non-empty buckets, 8-bucket warmup, and bucket-close confirmation.
...
- Player-prop disagreement alert reads are now split out of the giant shared live repository into a dedicated player-prop alert repository module, with alert-selection helpers moved into focused shared support code so the live-only alert math stays stable while the storage layer gets less tangled.
...
- Whole-board volatility now runs through one shared board-vw runtime across live alerts, replay, inspect, and desk surfaces, with the board-first fold preserved for same-window player follow-up.
...
## In Progress
- Live math note, 2026-05-21 18:36 MDT: the live board detector now matches the bake-off winner instead of the older phase-baseline/Kalman path. The shared runtime is board-vw with 60-second whole-board quote buckets, trailing `median + 3*MAD` over the prior 20 non-empty buckets, an 8-bucket warmup, 30-minute default context, and replay evaluation on every tick so bucket-close fires are not missed when no new row lands exactly on the confirm second.
- Math contract note, 2026-05-21 20:41 MDT: treat `outputs/innovation-team-suspend-signal-report/report.html` as the canonical explanation of the intended whole-board and off-price detector math when refactoring alert/runtime code; structural cleanup should preserve that behavior unless a deliberate math change is being made and revalidated.
...
- Re-validating the live board-vw path on localhost and the active game feed so the board-first fold, replay, and desk presentation all match the actual API output
