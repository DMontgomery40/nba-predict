...
- Live research data must come from persisted `games`, `game_states`, `nba_play_by_play_actions`, `market_instruments`, `source_markets`, `quote_ticks`, `market_microstructure_events`, `raw_payloads`, `adapter_runs`, `mapping_resolutions`, and `game_outcomes`.
...
This project is indexed by GitNexus as **nba-predict** (6076 symbols, 11285 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource                                     | Use for                                  |
| -------------------------------------------- | ---------------------------------------- |
| `gitnexus://repo/nba-predict/context`        | Codebase overview, check index freshness |
| `gitnexus://repo/nba-predict/clusters`       | All functional areas                     |
| `gitnexus://repo/nba-predict/processes`      | All execution flows                      |
| `gitnexus://repo/nba-predict/process/{name}` | Step-by-step execution trace             |
